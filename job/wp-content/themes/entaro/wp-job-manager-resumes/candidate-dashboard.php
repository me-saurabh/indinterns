...
<div id="resume-manager-candidate-dashboard">
	<p><?php echo _n( 'Your resume can be viewed, edited or removed below.', 'Your resume(s) can be viewed, edited or removed below.', resume_manager_count_user_resumes(), 'entaro' ); ?></p>
	<table class="resume-manager-resumes">
		...
		<tbody>
			<?php if ( ! $resumes ) : ?>
				<tr>
					<td colspan="<?php echo sizeof( $candidate_dashboard_columns ); ?>"><?php esc_html_e( 'You do not have any active resume listings.', 'entaro' ); ?></td>
				</tr>
			<?php else : ?>
							...
							<td class="<?php echo esc_attr( $key ); ?>">
									...
									<ul class="candidate-dashboard-actions">
										<?php
											...
											switch ( $resume->post_status ) {
												case 'publish' :
													$actions['edit'] = array( 'label' => esc_html__( 'Edit', 'entaro' ), 'nonce' => false );
													$actions['hide'] = array( 'label' => esc_html__( 'Hide', 'entaro' ), 'nonce' => true );
												break;
												case 'hidden' :
													$actions['edit'] = array( 'label' => esc_html__( 'Edit', 'entaro' ), 'nonce' => false );
													$actions['publish'] = array( 'label' => esc_html__( 'Publish', 'entaro' ), 'nonce' => true );
												break;
												case 'expired' :
													if ( get_option( 'resume_manager_submit_resume_form_page_id' ) ) {
														$actions['relist'] = array( 'label' => esc_html__( 'Relist', 'entaro' ), 'nonce' => true );
													}
												break;
											}

											$actions['delete'] = array( 'label' => esc_html__( 'Delete', 'entaro' ), 'nonce' => true );
								...
								<?php elseif ( 'date' === $key ) : ?>
									...
									if ( ! empty( $resume->_resume_expires ) && strtotime( $resume->_resume_expires ) > current_time( 'timestamp' ) ) {
										printf(__( 'Expires %s', 'entaro' ), date_i18n( get_option( 'date_format' ), strtotime( $resume->_resume_expires ) ) );
									} else {
			...
			<tfoot>
				<tr>
					<td colspan="<?php echo sizeof( $candidate_dashboard_columns ); ?>">
						<a class="btn btn-sm btn-success" href="<?php echo esc_url( get_permalink( $submit_resume_form_page_id ) ); ?>"><?php esc_html_e( 'Add Resume', 'entaro' ); ?></a>
					</td>
				</tr>
			</tfoot>
		<?php endif; ?>
